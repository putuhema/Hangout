      ...
      <div className="flex flex-col w-full items-center lg:flex-row">
        <div className="w-[640px] lg:w-[calc((100%_-_350px)_-_45px)] mr-0 lg:mr-[45px] h-[1000px] px-6 sm:p-2">
          <div className="flex overflow-x-auto gap-4 w-[640px] lg:w-full justify-between">
            {
              categories.map(category => (<Category key={category} imgUrl="/placeholder.jpeg" title={category} />))
            }
          </div>
          <div className="mt-8">
            <Tabs defaultValue="all" className="w-full">
              <TabsList className="flex justify-start gap-2">
                {
                  tabs.map(tab => (
                    <TabsTrigger className="p-2 px-4 rounded-full hover:bg-blue-100 data-[state=active]:bg-blue-100" key={tab.value} value={tab.value}>{tab.text}</TabsTrigger>
                  ))
                }
              </TabsList>
