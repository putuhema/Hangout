...
const Home = () => {
  ...
  const { data: today } = useQueryTab(
    "filter/today",
    "/f?date=0",
    tab === "Today"
  )
  const { data: thisWeek } = useQueryTab(
    "filter/thisWeek",
    "/f?date=7",
    tab === "This Week"
  )
  const { data: online } = useQueryTab(
    "filter/online",
    "/location?loc=isOnline&value=online",
    tab === "Online"
  )
  const { data: free } = useQueryTab(
    "filter/free",
    "/f?price=free",
    tab === "Free"
  )

  const { data: userEvent } = useQueryTab(
    `event/${userId}`,
    `/user?id=${userId}`,
    true
  )
