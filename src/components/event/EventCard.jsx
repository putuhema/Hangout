...
const EventCard = ({ event }) => {
    ...
    return (
        <div className="rounded-lg bg-gradient-to-r from-blue-100 to-red-100 relative w-full h-[250px] z-[1]">
            ...
            <Heart className="absolute top-2 right-2 text-white cursor-pointer transition-all ease-in-out duration-100" size={20} fill={`${isFavorite ? '#fff' : 'transparent'}`}
            ...
            <div className="absolute w-[calc(100%_-_16px)] p-2 bottom-2 left-2 bg-white/30 border border-white backdrop-blur-lg rounded-md">
                <Link to={`/event/${event.id}`} className="font-bold">{event.name}</Link>
                <span className="flex justify-between items-center mt-2 gap-2">
                    ...
                    <Badge className={`text-xs ${event.type === 'free' && 'bg-transparent text-black border-black hover:bg-black hover:text-white'}`}>{event.type === 'paid' ? formatToUnits(parseInt(event.price)) : event.type}</Badge>
                </span>
            </div>
        </div>)
}
