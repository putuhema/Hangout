...
const EventDetails = () => {
  ...
  const { data: currentUser, isFetched: isCurrentUserFetched, isLoading: userLoading } = useCurrentUser(userId)
  const { data: event, isFetched, isLoading: eventLoading } = useQuery({
    ...
    <Container>
      <div className="flex flex-col gap-4 pb-10">
        ...
        <div className="relative h-[350px] w-full">
          ...
          {
            eventLoading ? <Skeleton className="w-full h-[200px] rounded-md" /> : (
              <img
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="order-2 md:order-1 flex flex-col flex-1">
            ...
            <div className="w-full p-2 mt-4 rounded-md bg-secondary flex items-center justify-between">
              ...
              {
                userLoading || eventLoading ? <Skeleton className="w-[110px] h-[40px] rounded-full" /> :
                  <Follow eventId={eventId} userId={currentUser.id} userEventId={event.user.id} isFollowing={isFollowing} />
              }
            </div>
